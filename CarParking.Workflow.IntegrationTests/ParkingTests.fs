...
let cleanDb (cpdc, _) =
    ...
    conn.Execute("delete from Parking") |> ignore
    conn.Execute("delete from Payment") |> ignore
...
type ParkingWorkflowTests() =
    ...
    [<Property(MaxTest = 2000, Parallelism = 8)>]
    ...
    //task {
    //    let arrivalDates = Arb.generate<DateTime> |> Gen.sample 2000

    //    // clean db
    //    do! cleanDb <| createDctx ()

    //    // create a bunch of started parkings
    //    let! parkings = 
    //        arrivalDates
    //        |> Array.map(fun arrivalDate -> createNewParking (createDctx ()) arrivalDate)
    //        |> Task.WhenAll
        
    //    Assert.Equal(Array.length parkings, Array.length arrivalDates)

    //    let! loadParkings = 
    //        parkings
    //        |> Array.map(function
    //            | StartedFree prk ->
    //                let id = prk.Id |> ParkingId.toString
    //                getParking (createDctx ()) id
    //            | _ as prk ->
    //                Assert.True(false, sprintf "Expected StartedFree but actual %A" prk)
    //                Ok prk |> Task.FromResult)
    //        |> Task.WhenAll

    //    Assert.Equal (Array.length loadParkings, Array.length parkings)

    //    loadParkings
    //    |> Array.iter(fun result ->
    //        Result.map (fun parking ->
    //            Assert.True (Array.exists (fun p -> p = parking) parkings)
    //            ) result
    //        |> Result.mapError (fun err ->
    //            Assert.True(false, sprintf "Error %A" err))
    //        |> ignore)
    //}

//[<Property(MaxTest = 2000, Parallelism = 4)>]
//let ``Should create a StartedFreeParking`` (arrivalDate: DateTime) =
//    taskResult {
//        let dctx = createDctx ()
...
//*)
    ...
    [<Property(MaxTest = 2000, Arbitrary = [| typeof<ArbitaryFreeParkingDates> |], Parallelism = 8)>]
...
//[<Property(MaxTest = 2000, Arbitrary = [| typeof<ArbitaryExpiredParkingDates> |], Parallelism = 4)>]
